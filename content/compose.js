...
var mailmerge = {
	...
	update: function(x) {
		...
		document.getElementById("mailmerge-progress").value = x;
			...
			try {
				...
				for(var j = 0; j < accounts.length; j++) {
					...
					if(account[1] && account[5] && account[6] != "SEARCH") {
						...
						card = cardbookRepository.cardbookDisplayCards[account[4]].filter(function(element) { for(var i = 0; i < element.email.length; i++) { if(element.email[i][0][0].toLowerCase() == arrMatches.toLowerCase()) { return true; } } })[0];
						if(card) { break; }
	...
	compose: function() {
		...
		try {
			window.setTimeout(function() { mailmerge.send(from, to, cc, bcc, reply, subject, body, attachments, at); }, pause);
		}
		catch(e) {
			mailmerge.compose();
		}
	...
	send: function(from, to, cc, bcc, reply, subject, body, attachments, at) {
		try {
			/* update start */
			mailmerge.update("send");
			/* update end */
			...
			/* otherrandomheaders start */
			try {
				
				/* Thunderbird */
				var headers = Services.prefs.getCharPref("mail.compose.other.header").split(",");
				for(var i = 0; i < headers.length; i++) {
					
					var header = headers[i];
					
					if(gMsgCompose.compFields.hasHeader(header)) {
						compFields.setHeader(header, gMsgCompose.compFields.getHeader(header));
					}
					
				}
				
			} catch(e) {}
			/* otherrandomheaders end */
